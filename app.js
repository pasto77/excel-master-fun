document.addEventListener('DOMContentLoaded', () => {
    ...
    const content = {
        ...
        6: { 
            t: "Défi : La Moyenne", 
            c: "Quelle formule permet de calculer la moyenne des cellules A1 à A10 ?", 
            quiz: true,
            answer: "=MOYENNE(A1:A10)",
            r: "Correct ! +50 XP" 
        }
    };
        ...
        li.onclick = () => {
            ...
            if (item && item.quiz) {
                display.innerHTML = `
                    ...
                    <input type="text" id="quiz-input" placeholder="Tapez ici..." style="width:70%; padding:10px;">
                    <button id="btn-quiz" style="padding:10px; background:#217346; color:white; border:none; cursor:pointer;">Vérifier</button>
                    <p id="feedback" style="margin-top:15px; font-weight:bold;"></p>
                `;
                document.getElementById('btn-quiz').onclick = () => {
                    const val = document.getElementById('quiz-input').value.toUpperCase().trim();
                    const isOk = (val === item.answer.toUpperCase());
                ...
                display.innerHTML = `
                    ...
                    <button onclick="document.getElementById('feedback').textContent='${item.r}'" style="padding:10px; background:#217346; color:white; border:none; cursor:pointer;">${item.a}</button>
